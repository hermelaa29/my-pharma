...
import {
  Pill, Upload, Download, Plus, Trash2, Save, AlertCircle, CheckCircle2, Loader2, Edit2, X, Filter, ChevronLeft, ChevronRight, FileSpreadsheet, ChevronDown, Calendar as CalendarIcon, Package, TrendingUp, Boxes
} from 'lucide-react';
...
function CustomDatePicker({ value, onChange, placeholder, error }: { value: string, onChange: (v: string) => void, placeholder: string, error?: boolean }) {
  ...
  useEffect(() => {
    if (open && btnRef.current) {
      ...
      // If there's no room below, pop it upwards
      if (spaceBelow < 350) {
...
const Medicine: React.FC = () => {
  ...
  async function handleUpdateMedicine() {
    ...
    try {
      ...
      const res = await fetch(`/api/stores/${storeId}/medicines/${editingId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify(body),
      });
      ...
      setMedicines(prev => prev.map(m => m.id === editingId ? data.medicine : m));
      setEditingId(null);
      showToast('success', 'Medicine updated successfully!');
    } catch (err: any) {
      showToast('error', err.message || 'Error updating medicine.');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDeleteMedicine(id: string) {
    ...
    try {
      const res = await fetch(`/api/stores/${storeId}/medicines/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
      if (!res.ok) {
      ...
      showToast('error', err.message || 'Error deleting medicine.');
    }
  }

  function startEdit(med: MedicineRow) {
    setEditingId(med.id);
    setEditForm({
      itemCode: med.itemCode,
      itemName: med.itemName,
      receivedDate: med.receivedDate ? med.receivedDate.split('T')[0] : '',
      category: med.category || '',
      wholesaler: med.wholesaler || '',
      quantityReceived: med.quantityReceived,
      quantityIssued: med.quantityIssued,
      lossAdjustment: med.lossAdjustment,
      expireDate: med.expireDate ? med.expireDate.split('T')[0] : '',
  ...
  async function handleImportSubmit() {
    ...
    try {
      ...
      const res = await fetch(`/api/stores/${storeId}/medicines/bulk`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
  ...
  const categoryStats = React.useMemo(() => {
    ...
    medicines.forEach(med => {
      const cat = med.category?.trim() || 'Uncategorized';
      if (!stats[cat]) stats[cat] = { count: 0, totalQty: 0, totalValue: 0 };
      stats[cat].count += 1;
      stats[cat].totalQty += med.currentQuantity;
      stats[cat].totalValue += (med.currentQuantity * med.purchasePrice);
    });
  ...
  const totalStock = React.useMemo(() => {
    return medicines.reduce((acc, m) => ({
      count: acc.count + 1,
      totalQty: acc.totalQty + m.currentQuantity,
      totalValue: acc.totalValue + (m.currentQuantity * m.purchasePrice),
  ...
  const currentData = filteredMedicines.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
            ...
            <tbody>
              ...
              {!editingId && (
                <tr className="border-b border-[#51a22e]/10 bg-[#f6fbf3] hover:bg-[#edf7e8] transition-colors group">
                  ...
                  <td className="px-2 py-2 border-r border-gray-100"><input type="text" value={newRow.itemCode} onChange={e => handleChange('itemCode', e.target.value)} placeholder="MED-001" className={inputCls} /></td>
                  <td className="px-2 py-2 border-r border-gray-100"><input type="text" value={newRow.itemName} onChange={e => handleChange('itemName', e.target.value)} placeholder="Amoxicillin" className={inputCls} /></td>
                  ...
                  <td className="px-2 py-2 border-r border-gray-100"><input type="text" value={newRow.category ?? ''} onChange={e => handleChange('category', e.target.value)} placeholder="Antibiotic" className={inputCls} /></td>
                  <td className="px-2 py-2 border-r border-gray-100"><input type="text" value={newRow.wholesaler ?? ''} onChange={e => handleChange('wholesaler', e.target.value)} placeholder="PharmaCo" className={inputCls} /></td>
                  ...
                  <td className="px-3 py-2 sticky right-0 bg-[#f6fbf3] group-hover:bg-[#edf7e8] z-10 flex justify-center border-l border-gray-100">
                    <button onClick={handleAddMedicine} disabled={saving} className="flex items-center justify-center p-1.5 rounded-lg bg-[#51a22e] text-white hover:bg-[#459926] disabled:opacity-60 transition-all shadow-sm">
                      {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                    </button>
                  </td>
                </tr>
              ...
              {/* ── EXPLICIT DATA ROWS ────────────────────────────────────────────── */}
              {!loading && currentData.map((med, idx) => {
                const isEditing = editingId === med.id;
                const isLow = med.currentQuantity <= 5 && med.currentQuantity > 0;
                const isOut = med.currentQuantity <= 0;
                const profit = Number(med.salePrice) - Number(med.purchasePrice);
                const globalIdx = (currentPage - 1) * itemsPerPage + idx + 1;
                const monthStatus = getMonthLeftStatus(isEditing ? editForm.expireDate : med.expireDate);

                if (isEditing) {
                  return (
                    <tr key={med.id} className="border-b border-[#51a22e]/30 bg-[#fefce8]">
                      ...
                      <td className="px-3 py-2 sticky right-0 bg-[#fefce8] z-10 border-l border-gray-100">
                        <div className="flex items-center justify-center gap-2">
                          <button onClick={handleUpdateMedicine} disabled={saving} className="p-1.5 rounded-lg bg-amber-500 text-white hover:bg-amber-600 disabled:opacity-60 transition-all shadow-sm">
                            {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                          </button>
                ...
                return (
                  <tr key={med.id} className={`border-b border-gray-100 transition-colors group ${
                    ...
                    <td className="px-3 py-2.5 border-r border-gray-100"><span className="text-xs font-bold text-[#51a22e]">{med.itemCode}</span></td>
                    <td className="px-3 py-2.5 border-r border-gray-100"><span className="text-xs font-semibold text-[#5b5b5b]">{med.itemName}</span></td>
                    <td className="px-3 py-2.5 border-r border-gray-100"><span className="text-xs text-[#5b5b5b]/70">{formatDate(med.receivedDate)}</span></td>
                    <td className="px-3 py-2.5 border-r border-gray-100">
                      {med.category ? <span className="inline-block px-2 py-0.5 rounded-md bg-[#51a22e]/8 border border-[#51a22e]/15 text-[10px] font-bold text-[#51a22e] uppercase tracking-wide">{med.category}</span> : <span className="text-[#5b5b5b]/30 text-xs">—</span>}
                    </td>
                    <td className="px-3 py-2.5 border-r border-gray-100"><span className="text-xs text-[#5b5b5b]/70">{med.wholesaler || '—'}</span></td>
                    <td className="px-3 py-2.5 border-r border-gray-100 text-right"><span className="text-xs font-bold text-[#5b5b5b]">{med.quantityReceived}</span></td>
                    <td className="px-3 py-2.5 border-r border-gray-100 text-right"><span className="text-xs text-[#5b5b5b]/70">{med.quantityIssued}</span></td>
                    <td className="px-3 py-2.5 border-r border-gray-100 text-right"><span className="text-xs text-red-400">{med.lossAdjustment > 0 ? `-${med.lossAdjustment}` : med.lossAdjustment}</span></td>
                    <td className="px-3 py-2.5 border-r border-gray-100"><span className="text-xs text-[#5b5b5b]/70">{formatDate(med.expireDate)}</span></td>
                    ...
                    <td className="px-3 py-2.5 border-r border-gray-100"><span className="text-xs font-mono text-[#5b5b5b]/70">{med.batchNumber || '—'}</span></td>
                    <td className="px-3 py-2.5 border-r border-gray-100 text-right"><span className="text-xs font-semibold text-[#5b5b5b]">${Number(med.purchasePrice).toFixed(2)}</span></td>
                    <td className="px-3 py-2.5 border-r border-gray-100 text-right"><span className="text-xs font-bold text-[#51a22e]">${Number(med.salePrice).toFixed(2)}</span></td>
                    ...
                    <td className={`px-3 py-2.5 sticky right-0 z-10 border-l border-gray-100 ${
                      ...
                      <div className="flex items-center justify-center gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => startEdit(med)} className="p-1.5 rounded-lg text-blue-500 hover:bg-blue-50 hover:text-blue-600 transition-colors" title="Edit">
                          <Edit2 className="w-4 h-4" />
                        </button>
                        <button onClick={() => handleDeleteMedicine(med.id)} className="p-1.5 rounded-lg text-red-500 hover:bg-red-50 hover:text-red-600 transition-colors" title="Delete">
